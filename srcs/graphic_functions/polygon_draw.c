...
void VID_HLine(int x1, int x2, int y, int c, t_fdf *fdf)
{
	int		x;
	...
	for(x=x1; x<=x2; x++)
		put_pixel(fdf, x, y, c); //add *fdf ^
}


void		polygon_triangle(t_2d p[3], int c, t_fdf *fdf)
{
	t_2d	p1d, p2d, p3d;
	...
	if(yd1)
	{
		...
		while (++i <= p2d.y)
		{
			...
			if(lx != rx)
				VID_HLine(MIN(lx,rx), MAX(lx,rx), i, c, fdf);
		}
	}
...
void		polygon_draw(t_poligon_2d *polygon, t_fdf *fdf)
{
	t_2d	p[3];
	int		i;

//	ft_printf("%#X ", polygon->texture);
	...
	while (i < polygon->points_count - 1)
	{
		...
		polygon_triangle(p, polygon->texture, fdf);
		i++;
	}
}
